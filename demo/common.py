...
def generate_questions(
    ...
    *,
    ...
    preferred_unique_phrases = sorted(
        (
            (paper_id, phrase)
            for paper_id, phrases in per_paper_phrases.items()
            for phrase in phrases
            if len(phrase_docs.get(phrase.lower(), set())) == 1
            and phrase_counts.get(paper_id, {}).get(phrase.lower(), 0) >= 2
        ),
        key=lambda item: (item[0], len(item[1])),
    )
    unique_phrases = preferred_unique_phrases or sorted(
        (
            (paper_id, phrase)
            for paper_id, phrases in per_paper_phrases.items()
            for phrase in phrases
    ...
    for paper_id, phrase in unique_phrases:
        ...
        q_index += 1
        if len(questions) >= max(10, question_count - max(5, question_count // 5)):
            break
