...
class ComputeEconomicsBenchmark:
    ...
    def benchmark_dual_stream(
        ...
        for seq_len in sequence_lengths:
            ...
            # Mamba: O(N * d^2) where d is model dimension
            # Approximately: N * (d^2 * 2) for matrix multiplies
            d = engine.config.d_model
            n_layers = engine.config.n_layer
            estimated_flops = seq_len * (d ** 2 * 2) * n_layers
    ...
    def benchmark_transformer_baseline(
        ...
        for seq_len in sequence_lengths:
            ...
            # Plus O(N * d^2) for FFN
            d = model.config.n_embd
            n_layers = model.config.n_layer
            n = input_ids.size(1)
            attention_flops = n_layers * (n ** 2 * d)
            ffn_flops = n_layers * (n * d ** 2 * 4)  # FFN has 4*d intermediate
            estimated_flops = attention_flops + ffn_flops
    ...
    def plot_comparison(
        ...
        tf_time_per_token = [r.time_per_token * 1000 if r.time_per_token != float('inf') else None 
                             for r in transformer_results]
        tf_memory = [r.memory_mb if r.memory_mb != float('inf') else None 
                     for r in transformer_results]
        tf_flops = [r.estimated_flops if r.estimated_flops != float('inf') else None 
                    for r in transformer_results]

        # Filter out None values for transformer
        tf_valid_lens = [l for l, t in zip(tf_seq_lens, tf_time_per_token) if t is not None]
        ...
        plt.subplot(1, 2, 1)
        plt.plot(ds_seq_lens, ds_time_per_token, 'o-', label='Dual-Stream (O(N))', linewidth=2)
        if tf_valid_lens:
            plt.plot(tf_valid_lens, tf_valid_time, 's-', label='Transformer (O(N²))', linewidth=2)
        ...
        if tf_valid_lens:
            plt.plot(tf_valid_lens, tf_valid_memory, 's-', label='Transformer', linewidth=2)
        plt.xlabel('Sequence Length (tokens)')
        ...
        if tf_valid_lens:
            plt.plot(tf_valid_lens, tf_valid_flops, 's-', label='Transformer (O(N²))', linewidth=2)
        plt.xlabel('Sequence Length (tokens)')
...
def parse_args():
    ...
    parser.add_argument("--baseline-model", type=str, default="gpt2-medium", help="Baseline model")
    parser.add_argument("--prompt", type=str, default="The quick brown fox", help="Test prompt")
    parser.add_argument(
        "--sequence-lengths",
        type=int,
        nargs="+",
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("output/benchmarks"), help="Output directory")
    parser.add_argument("--skip-baseline", action="store_true", help="Skip transformer baseline")
...
def main():
        ...
        results["transformer"] = [
            {
                ...
                "ttft": r.time_to_first_token if r.time_to_first_token != float('inf') else None,
                "time_per_token": r.time_per_token if r.time_per_token != float('inf') else None,
                "total_time": r.total_time if r.total_time != float('inf') else None,
                "tokens_per_second": r.tokens_per_second,
                "estimated_flops": r.estimated_flops if r.estimated_flops != float('inf') else None,
            }
            for r in transformer_results
        ]
    else:
    ...
    if dual_stream_results:
        ds_100k = next((r for r in dual_stream_results if r.sequence_length == 100000), None)
        if ds_100k:
    ...
    if transformer_results:
        tf_100k = next((r for r in transformer_results if r.sequence_length == 100000), None)
        if tf_100k:
