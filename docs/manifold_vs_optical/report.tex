...
Using 512~byte windows, a 384~byte stride, and a 9~byte payload per unique signature, the proposed encoder delivers \textbf{44--46$\times$ byte} and \textbf{85--99$\times$ token compression} on the full benchmarks while retaining \textbf{87--92\% token accuracy} and \textbf{$7$--$11\%$ normalized edit distance}.
Hazard-gated verification maintains 100\% recall with \textbf{2--5\%} false-positive rates and dataset-level precision between \textbf{3--17\%}, and the entire run finishes in under one hour on a single RTX~3080~Ti.
...
\begin{itemize}
    ...
    \item We integrate hazard-gated verification into the compression loop, yielding perfect recall with 2--5\% false-positive rates and dataset-level precision in the 3--17\% range on the full Fox and OmniDocBench benchmarks.
...
All corpora land between 44$\times$ and 46$\times$ byte compression, with 85--90$\times$ token compression and 87--92\% token accuracy; normalized edit distances remain in the 7--11\% band despite the lossy signatures.
Hazard verification maintains perfect recall, but cross-document precision ranges from 3\% on OmniDoc to 17\% on Fox because we deliberately deduplicate every recurring window; the resulting false-positive rates stay between 2--5\%.
...
\begin{table}[h]
    ...
    \begin{tabular}{lrrrrrr}
        ...
        \midrule
        Fox EN (112)  & 44.29 & 85.48 & 91.67\% & 7.25\% & 16.44\% & 4.58\% \\
        Fox CN (100)  & 44.68 & 88.08 & 90.55\% & 9.15\% & 16.60\% & 5.08\% \\
        OmniDoc (1\,349) & 45.93 & 89.49 & 87.16\% & 10.59\% & 3.36\% & 2.12\% \\
        \bottomrule
    \end{tabular}
\end{table}
