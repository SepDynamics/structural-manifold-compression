...
// Load candles from Valkey using schema md:candles:{instrument}:M1 and date
// range YYYY-MM-DD..YYYY-MM-DD
std::vector<sep::Candle>
load_candles_from_valkey_spec(const std::string &spec) {
  ...
  if (parts.size() < 3) {
    spdlog::error("Invalid valkey spec '{}', expected "
                  "valkey:{{instrument}}:{{start}}:{{end}}",
                  spec);
    return {};
  }
  ...
  std::sort(candles.begin(), candles.end(),
            [](const sep::Candle &a, const sep::Candle &b) {
...
int main(int argc, char **argv) {
  cxxopts::Options opts("manifold_generator",
                        "Generate manifolds from candle data");
  opts.add_options()("input", "file or valkey:{instrument}:{start}:{end}",
                     cxxopts::value<std::string>())(
      "output", "file path or 'valkey'",
      cxxopts::value<std::string>()->default_value(""))(
      "cpu-only", "Force CPU only",
  ...
  if (!out.empty() && out == "valkey") {
    ...
    char buf[32]{0};
    if (gmt) {
      std::snprintf(buf, sizeof(buf), "%04d-%02d-%02d", gmt->tm_year + 1900,
                    gmt->tm_mon + 1, gmt->tm_mday);
    } else {
      std::snprintf(buf, sizeof(buf), "1970-01-01");
    }
    std::string day_key(buf);
    std::string instr = manifold.value(
        "instrument", instrument_hint.empty() ? "UNKNOWN" : instrument_hint);
