...
ByteStreamConfig build_config(const cxxopts::ParseResult &args) {
  ...
  const size_t window_bits_from_bits =
      args.count("window-bits") ? args["window-bits"].as<size_t>() : 0;
  const size_t window_bits_from_bytes =
      args.count("window-bytes") ? args["window-bytes"].as<size_t>() * 8 : 0;
  ...
  const size_t step_bits_from_bits =
      args.count("step-bits") ? args["step-bits"].as<size_t>() : 0;
  const size_t step_bits_from_bytes =
      args.count("step-bytes") ? args["step-bytes"].as<size_t>() * 8 : 0;
  ...
  if (args.count("signature-precision")) {
    config.signature_precision =
        std::clamp(args["signature-precision"].as<int>(), 0, 6);
  }

  if (args.count("coherence-threshold")) {
    config.structural_options.coherence_threshold =
        args["coherence-threshold"].as<double>();
  }
  if (args.count("stability-threshold")) {
    config.structural_options.stability_threshold =
        args["stability-threshold"].as<double>();
  }
  if (args.count("collapse-threshold")) {
    config.structural_options.collapse_threshold =
        args["collapse-threshold"].as<double>();
  }
  if (args.count("max-iterations")) {
    config.structural_options.max_iterations =
        static_cast<int>(args["max-iterations"].as<size_t>());
  }
  if (args.count("disable-damping")) {
    config.structural_options.enable_damping =
        !args["disable-damping"].as<bool>();
  }
  if (args.count("damping-factor")) {
    config.structural_options.damping_factor =
        args["damping-factor"].as<double>();
  }
  if (args.count("entropy-weight")) {
    config.structural_options.entropy_weight =
        args["entropy-weight"].as<double>();
  }
  if (args.count("coherence-weight")) {
    config.structural_options.coherence_weight =
        args["coherence-weight"].as<double>();
  }
...
std::vector<uint8_t> load_bytes(const CLIConfig &cli) {
  try {
    ...
    throw std::runtime_error(std::string("Failed to load byte stream: ") +
                             ex.what());
  }
}
...
int main(int argc, char **argv) {
  // Configure spdlog to write to stderr so stdout remains clean for JSON
  // formatting
  try {
  ...
  cxxopts::Options options("byte_stream_manifold",
                           "Run QFH/SRI analysis on arbitrary byte streams");

  options.add_options()("input", "Path to input file",
                        cxxopts::value<std::string>())(
      "fd", "File descriptor/socket to read (defaults to stdin when omitted)",
      cxxopts::value<int>())("max-bytes",
                             "Maximum bytes to read from input (0 = all)",
                             cxxopts::value<size_t>()->default_value("0"))(
      "window-bits", "Sliding window length in bits", cxxopts::value<size_t>())(
      "window-bytes", "Sliding window length in bytes",
      cxxopts::value<size_t>())("step-bits", "Stride between windows in bits",
                                cxxopts::value<size_t>())(
      "step-bytes", "Stride between windows in bytes",
      cxxopts::value<size_t>())("max-windows",
                                "Cap the number of windows analysed (0 = all)",
                                cxxopts::value<size_t>()->default_value("0"))(
      "msb-first", "Interpret bytes MSB first instead of LSB first",
      cxxopts::value<bool>()->default_value("false"))(
      "repetition-lookback",
      "Lookback horizon (in windows) for repetition counting (0 = entire run)",
      cxxopts::value<size_t>()->default_value("0"))(
      "signature-precision", "Decimal places used when bucketing signatures",
      cxxopts::value<int>()->default_value("2"))(
      "coherence-threshold", "QFH coherence collapse threshold",
      cxxopts::value<double>())("stability-threshold",
                                "QFH stability collapse threshold",
                                cxxopts::value<double>())(
      "collapse-threshold", "QFH rupture collapse threshold",
      cxxopts::value<double>())(
      "max-iterations", "Maximum iterations for QFH analyser",
      cxxopts::value<size_t>())("disable-damping", "Disable trajectory damping",
                                cxxopts::value<bool>()->default_value("false"))(
      "damping-factor", "Custom damping factor", cxxopts::value<double>())(
      "entropy-weight", "Weight applied to entropy when computing lambda",
      cxxopts::value<double>())(
      "coherence-weight",
      "Weight applied to (1 - coherence) when computing lambda",
      cxxopts::value<double>())(
      ...
      cxxopts::value<bool>()->default_value("false"))("help", "Show usage");
  ...
  spdlog::info("Analysed {} windows across {} bytes ({} bits).",
               manifold.summary.total_windows, manifold.summary.analysed_bytes,
               manifold.summary.analysed_bits);
  spdlog::info("Mean coherence {:.4f}, entropy {:.4f}, lambda {:.4f}.",
               manifold.summary.mean_coherence, manifold.summary.mean_entropy,
