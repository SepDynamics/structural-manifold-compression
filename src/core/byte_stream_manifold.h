...
struct ByteStreamConfig {
  ...
  size_t max_windows =
      0; // Optional cap on number of windows to analyse (0 = unlimited).
  bool lsb_first =
      true; // Interpret bytes as LSB-first when converting to bits.
  size_t repetition_lookback =
      0; // Optional lookback (in windows) for repetition counts (0 = keep all).
  sep::structural::StructuralOptions
      structural_options{};    // Tunable Structural Entropy parameters.
...
[[nodiscard]] std::vector<uint8_t> load_bytes_from_file(const std::string &path,
                                                        size_t max_bytes = 0);
[[nodiscard]] std::vector<uint8_t> load_bytes_from_fd(int fd,
                                                      size_t max_bytes = 0);
[[nodiscard]] std::vector<uint8_t> load_bytes_from_buffer(const uint8_t *data,
                                                          size_t length);
